[Begin_ResourceLayout]
	...
	[Directives:HeightMap HEIGHT_OFF HEIGHT]
	...
	cbuffer Parameters : register(b2)
	{
		...
		float 	HeightAmount		: packoffset(c5.x); [Default(1.0)]
	};
	...
	Texture2D HeightTexture 				: register(t13);
	SamplerState HeightSampler				: register(s13);
...
[Begin_Pass:ShadowMap]
	...
	#if HEIGHT
		float height = HeightTexture.SampleLevel(HeightSampler, input.TexCoord0,0).r;
		float4 position = float4(input.Position + float3(0,height * HeightAmount,0), 1);
	#else
		float4 position = float4(input.Position,1);
	#endif
	
	 	const float4 transformedPosWorld = mul(position, World);
...
[Begin_Pass:ZPrePass]
	...
	{
		float3      Position            : POSITION;
	#if NORMAL || MT_RG_TEXTURED || (ATEST && DIFF) || HEIGHT
		float2      TexCoord0           : TEXCOORD0;
	#endif
	...
	{
	    float u = HeightTexture.SampleLevel(HeightSampler,  uv + texelSize * float2(0.0, -1.0), 0).r;
	    float r = HeightTexture.SampleLevel(HeightSampler,  uv + texelSize * float2(-1.0, 0.0),0).r;
	    float l = HeightTexture.SampleLevel(HeightSampler,  uv + texelSize * float2(1.0, 0.0),0).r;
	    float d = HeightTexture.SampleLevel(HeightSampler,  uv + texelSize * float2(0.0, 1.0),0).r;
	...
	VSOutputPbr VertexFunction(VSInputPbr input)
	{
		...
		output.NormalWS = mul(float4(input.Normal, 0.0), World).xyz;
	...
	{		
		...
		float3 normal =  getNormal(input.TexCoord0, texelSize);
	#else
...
[Begin_Pass:Default]
	...
	{
		float3      Position            : POSITION;
	#if DIFF || NORMAL || EMIS || MT_RG_TEXTURED || CLEAR_NORMAL || ANIS || AO || HEIGHT
		float2      TexCoord0           : TEXCOORD0;
	#endif
