[Begin_ResourceLayout]
	...
	[Directives:PointLight POINT_LIGHT_OFF POINT_LIGHT]
	[Directives:SpotLight SPOT_LIGHT_OFF SPOT_LIGHT]
	[Directives:AreaLight AREA_LIGHT_OFF AREA_LIGHT]
	...
	cbuffer Parameters : register(b3)
	{
		...
		float TilesWidth		: packoffset(c5.x); [Default(4)]
		float TilesHeight		: packoffset(c5.y); [Default(4)]
		float Speed				: packoffset(c5.z); [Default(4)]	
		float Tile				: packoffset(c5.w); [Default(0)]
		float2 RainOffset		: packoffset(c6.x); [Default(0.3,0.3)]
		float2 RainTile			: packoffset(c6.z); [Default(1,1)]
		float TimeOffset		: packoffset(c7.x); [Default(0.1)]
	};
	...
	cbuffer IrradianceSHBuffer : register(b5)
	{
		float4 IrradianceSH[9]		: packoffset(c0.x); [IrradianceSH]
	};
	
	cbuffer ShadowMapViewBuffer : register (b6)
	{
	...
	Texture2D RainTexture					: register(t13);
	SamplerState RainSampler				: register(s13);
	
	Texture2D RainOpacity					: register(t14);
	SamplerState RainOpacitySampler			: register(s14);
...
[Begin_Pass:Default]
	...
	void InitMaterial(const VSOutputPbr input, inout MaterialInputs material)
	{
		...
		float3 multipler = RainOpacity.Sample(RainOpacitySampler, input.TexCoord0).rgb;		
		
		// Rain Texture 1
		float2 normalizeTexCoords1 = fmod(input.TexCoord0 * RainTile,float2(1,1));		
		float Tile1 = fmod(floor(Time * Speed),  TilesWidth  * TilesHeight);
		float2 tileCount1 = float2(1.0.xx) / float2(TilesWidth, TilesHeight);
		float tileY1 = floor(Tile1 / 4);
		float tileX1 = fmod(Tile1, TilesWidth);
		float2 coords = (normalizeTexCoords1 + float2(tileX1, tileY1)) * tileCount1;
		float3 rayNormal1 = RainTexture.Sample(RainSampler, coords).rgb * 2 -1;
		rayNormal1 *= multipler;
		material.normal += rayNormal1;
		
		// Rain Texture 2
		float2 normalizeTexCoords2 = fmod((input.TexCoord0 + RainOffset) * RainTile,float2(1 ,1));
		float Tile2 = fmod(floor((Time +TimeOffset) * Speed),  TilesWidth  * TilesHeight);
		float2 tileCount2 = float2(1.0.xx) / float2(TilesWidth, TilesHeight);
		float tileY2 = floor(Tile2 / 4);
		float tileX2 = fmod(Tile2, TilesWidth);
		float2 coords2 = (normalizeTexCoords2 + float2(tileX2, tileY2)) * tileCount2;
		float3 rayNormal2 = RainTexture.Sample(RainSampler, coords2).rgb * 2 -1; 
		rayNormal2 *= multipler;
	    material.normal += rayNormal2;
		...
		float  metallicFromTexture = metalRoughness.r;
